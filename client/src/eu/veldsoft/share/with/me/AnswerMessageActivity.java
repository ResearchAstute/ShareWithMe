	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.replay_reject).setOnClickListener(
				...
				new View.OnClickListener() {
					...
					public void onClick(View view) {
						...
						Toast.makeText(AnswerMessageActivity.this, R.string.discard_replay, Toast.LENGTH_SHORT)
						.show();
		...
		(new AsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				String host = "";
				try {
					host = getPackageManager().getApplicationInfo(AnswerMessageActivity.this.getPackageName(),
							PackageManager.GET_META_DATA).metaData.getString("host");
				} catch (NameNotFoundException exception) {
					System.err.println(exception);
					return null;
				...
				String script = "";
				try {
					script = getPackageManager().getActivityInfo(AnswerMessageActivity.this.getComponentName(),
							PackageManager.GET_ACTIVITIES | PackageManager.GET_META_DATA).metaData.getString("script");
				} catch (NameNotFoundException exception) {
					System.err.println(exception);
					return null;
				}
				...
				try {
					 ...
					 */
					if (found == true) {
						((EditText) findViewById(R.id.message_read)).setText(message);
					}
				} catch (ClientProtocolException exception) {
					System.err.println(exception);
				} catch (IOException exception) {
